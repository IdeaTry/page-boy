...
function createStar() {
  ...
  var tag = $('<div class="star"></div>'),
    ...
    q = parseInt(Math.random() * 600 + 1500),
    n = parseInt(Math.random() * 10 + (-10)),
    winHeight = window._WINDOW_HEIGHT || (window._WINDOW_HEIGHT = $(window).height());

  tag.css({'left': o, 'top': n});
  tag.animate({'left': o - p, 'top': winHeight + 100}, q);
